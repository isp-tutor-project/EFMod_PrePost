...
import { THouseController } from "../../thermite/THouseController";
...
export class TIntroPart1 extends TScene
{
	...
	constructor()
	{
		...
		this.i1p1imgTool.on(TSelectEvent.WOZIMGSELECT,  this.doImageMap, this);			
		this.i1p1tabController.on(TSelectEvent.WOZTABSELECT,  this.doTabMap, this);			
		
		this.i1p1tabController.on("Done",  this.questionFinished);
	}
...
//***** Clickable image - this.i1p1imgTool actions
	...
	{			
		switch(evt.selection)
		{
			case "chimneyLeft":
			case "chimneyRight":
					this.i1p1tabController.topClick(new TMouseEvent("", CONST.BUTTON_CLICK));
					break;
					
			case "pinkPaint":
			case "bluePaint":
					this.i1p1tabController.centerClick(new TMouseEvent("", CONST.BUTTON_CLICK));
					break;
					
			case "singleDoor":
			case "doubleDoor":
					this.i1p1tabController.bottomClick(new TMouseEvent("", CONST.BUTTON_CLICK));
					break;
