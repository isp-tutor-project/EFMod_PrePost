...
export class TIntroPart2 extends TScene
{
	...
	constructor()
	{
		...
		//
		this.i1p2imgTool.on(TSelectEvent.WOZIMGSELECT,  this.doImageMap, this);			
		this.i1p2tabController.on(TSelectEvent.WOZTABSELECT,  this.doTabMap, this);
		
		// Display the changed tab if the image does not match the original
		this.i1p2tabController.this.useChanged = true;			
		
		this.i1p2tabController.on("Done",  this.questionFinished);			
	}
