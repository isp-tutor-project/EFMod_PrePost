...
export class TIntroPart1 extends TScene
{
	constructor()
	{
		...
		this.imgTool.on(TSelectEvent.WOZIMGSELECT,  this.doImageMap, this);			
		this.TabTool.on(TSelectEvent.WOZTABSELECT,  this.doTabMap, this);
	}
...
//***** Clickable image - imgTool actions
	...
	public doImageMap(evt:TSelectEvent)
	{
		switch(evt.selection)
		{
			case "chimneyLeft":
			case "chimneyRight":
					this.tabTool.topClick(new TMouseEvent("", CONST.BUTTON_CLICK));
					break;
					
			case "pinkPaint":
			case "bluePaint":
					this.tabTool.centerClick(new TMouseEvent("", CONST.BUTTON_CLICK));
					break;
					
			case "singleDoor":
			case "doubleDoor":
					this.tabTool.bottomClick(new TMouseEvent("", CONST.BUTTON_CLICK));
					break;
